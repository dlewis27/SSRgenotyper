...
**-M --LinkageMapFile** Outputs an "A,B,H" genotype file for downstream linkage mapping of biparental populations. The first two SAM files in the SAMFiles.txt list should correspond to two parents of the biparental population and are necessary for correctly phasing the genotypes. Non-informative (parent 1 and parent 2 have the same allele) markers and markers where there is no call for one or both the parents will be excluded. If one or both of the parents are heterozygous, the marker will also be excluded.\
