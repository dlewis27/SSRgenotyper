...
**-B --BorderSeq** The number of bordering (flanking) nucleotides on each side of the SSR that must match the reference for a read to be used to support a genotypic call. If there is a concern with non-specific read mapping (allopolyploidy, paralogous/duplicated loci), increasing this parameter can add specficity. [20] \
