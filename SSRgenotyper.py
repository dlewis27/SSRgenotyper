...
parser.add_argument("-S", "--Support", help = 'The minimum number of supporting reads needed for a genotype to be called at an SSR locus. If the total number of reads is less than this threshold, the genotype will not be called and will be reported as "0,-4". Between 0 and 1. [3]', type = int, default = 3)
...
parser.add_argument("-L", "--LinkageMapFile", help = 'Outputs an "A,B,H" genotype file for downstream linkage map construction of biparental populations. The first two SAM files in the SAMFiles.txt should correspond to two parents of the biparental population and are necessary for correctly phasing the genotypes. If the parental genotypes are monomorphic (same genotype) or if one of the parents is missing a genotypic call the SSR locus is excluded. Similarly, if one or both of the parents have a heterozygous genotype, phase cannot be determined, and the marker is also be excluded. SSRgenotyper can impute the genotype of a missing parent (the other parent must have a genotype) based on the progeny by providing the -L argument with a with a specific numeric threshold value (e.g., -L 0.30). If a threshold is provided and two alleles (major and minor) are found in the progeny with the minor allele frequency above the provided threshold and where one allele agrees with the genotyped parent, SSRgenotyper will assign the second allele as the genotype of the missing parent. [.30]', nargs='?', type=float, const=.3)
