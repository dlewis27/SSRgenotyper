...
def findRefRepeatHelper(s, sequence):
    reg = "".join(["((", s, ")+)"])
...
def findRefRepeat(sequence, repeatSize):
    ...
    parResults = [e.submit(findRefRepeatHelper, s, sequence) for s in generatedRepeats]
    for p in parResults:
...
def processSams(refData, outputDict, inSamFiles):
    ...
    def processSamsHelper(samFile):
        ...
        return samName, colToAppend
    
    e = concurrent.futures.ThreadPoolExecutor()
    results = e.map(processSamsHelper, inSamFiles)
